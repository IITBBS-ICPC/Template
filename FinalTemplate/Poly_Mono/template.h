...
private:
  constexpr static vector<__uint64_t> init_omegas() {
    ...
    for(int i = m + 1; i < max_base; ++i)
      res[i] = reduce(res[i - 1] * 1ll * omega);
    for(int i = m - 1; i; --i)
      res[i] = res[i << 1];
    res[0] = (1ll << 32) % mod;
    return res;
  ...
  constexpr static vector<__uint64_t> init_iomegas() {
    ...
    res[m] = (1ll << 32) % mod;
    for(int i = m + 1; i < max_base; ++i)
      res[i] = reduce(res[i - 1] * 1ll * iomega);
    for(int i = m - 1; i >= 0; --i)
      res[i] = res[i << 1];
    return res;
  }
  ...
  static const __uint32_t reduce(__uint64_t x) {
    __uint32_t t
      = (x + (((static_cast<__uint32_t>(x)) * N1) * N))
        >> 32;
    if(t >= mod) t -= mod;
    return t;
  }
...
public:
  ...
  static vector<int> mul(vector<int> a, vector<int> b) {
    ...
    ifft(a);
    __uint64_t f = ((((1ll << 32) / nbase) % mod) * 1ll
                    * omegas[0])
                   % mod;
    a.resize(need);
  ...
  static vector<int> inv(vector<int> &a) {
    ...
    while(k < n) {
      ...
      ifft(res);
      __uint64_t f = ((((1ll << 32) / need) % mod) * 1ll
                      * omegas[0])
                     % mod;
      f = (f << 32) % mod;
      res.resize(l);
...
const vector<__uint64_t>
  Ntt<max_base, mod, omega>::omegas = init_omegas();
template <int max_base, int mod, int omega>
const vector<__uint64_t>
  Ntt<max_base, mod, omega>::iomegas = init_iomegas();
